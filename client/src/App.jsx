...
import { Button, Card, Col, Image, Row, Typography, Slider } from "antd"
                ...
                <Card loading={loading} ho>
                  ...
                  {token.type === "audio" ? <Title level={5}>{token.info?.title}</Title> : <Title level={5}>{token.title}</Title>}
                  {
                    (playing && playing === token.id && token.type === "audio") ? <audio controls autoPlay className='w-full'>
                      ...
                      :
                      <Button onClick={() => setPlaying(token.id)} type='primary'>{
                        token.type === "video" ? <a href={token.contentURI} target="_blank">Open Video</a> : "Play"

                      }</Button>

                  }
                  <Button href={`/${token.id}`} type="link">Open</Button>
